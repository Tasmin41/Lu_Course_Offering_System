<?php
   ...
   $id = $_GET['id'];
   //echo $id;
   $dataFetchQuery = "SELECT * FROM `batch_59` WHERE id = '$id'";
			...
			<?php
              include '../config.php';
			    if(isset($_POST['submit'])){
                    ...
                    $updateQuery ="UPDATE batch_59 SET `course_code`= '$courseCode',`course_title`= '$courseTitle',`credit`= '$credit',`offer`= '$offered',`semester`='$semester' WHERE id = '$id'";
                    if(mysqli_query($conn,$updateQuery)){
                        echo "<script>alert('Course Updated!!! !!')</script>";
                        echo "<script>location.href='batch59_list.php'</script>";
                     }else{
